...
import module namespace ediarumBeacon="http://www.bbaw.de/telota/ediarum/beacon" at "../personen/beacon/functions.xql";
...
let $redirect := 
        if ($personid='')
        then (response:redirect-to(xs:anyURI(concat($edweb:baseURL, '/register/mythologie/index.xql'))))
        else ()            
...
let $header :=  
    <transferContainer>
        ...
        {transform:transform($person, doc('../personen/personen.xsl'), ())}
    </transferContainer>
...
let $output := edweb:transformHTML($header, $content, 'register', $person/tei:persName[@type='reg'][1]/tei:surname/text())
