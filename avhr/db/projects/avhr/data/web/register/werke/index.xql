...
let $werkliste := <ul>{
    for $werk in doc($edweb:dataRegisterWorks)//tei:bibl[starts-with(edwebRegister:orderLabel(./tei:title), $edwebRegister:section)]
...
let $header :=  
    <transferContainer>
        ...
        <h1>Werkregister - {$edwebRegister:section}</h1>
...
let $output := edweb:transformHTML($header, $content, 'register', 'Werkregister')
