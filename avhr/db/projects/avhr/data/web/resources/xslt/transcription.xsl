<xsl:stylesheet xmlns:exist="http://exist.sourceforge.net/NS/exist" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:telota="http://www.telota.de" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="#all" version="2.0">
    <!-- ##### Strukturelemente ##### -->
    <xsl:param name="id"/>
...
<xsl:text>http://digilib.bbaw.de/digitallibrary/greyskin/diginew.jsp?fn=silo10/avhr</xsl:text>
    ...
    <xsl:template match="tei:hi[@rendition='#aq']" mode="kritischerText faksimileText">
        <span class="tooltip antiqua">
            <span class="fussnote">
                <span class="fussnotenpfeil"> </span>
                <xsl:text>In lateinischer Schrift</xsl:text>
            </span>
            <xsl:apply-templates mode="#current"/>
        </span>
    </xsl:template>
        ...
        <xsl:choose>
            ...
            <xsl:when test="matches(@key, ' ')">
                ...
                <xsl:for-each select="tokenize(@key, ' ')">
                    ...
                    <xsl:variable name="personname" as="node()">
                        <xsl:copy-of select="doc('/db/projects/hirt/data/Register/Personen.xml')//tei:person[@xml:id=$personid]"/>
                    </xsl:variable>
                    <a href="../register/personen/detail.xql?id={.}">
    ...
    <xsl:template mode="#all" match="tei:orgName">
        ...
        <xsl:choose>
            <xsl:when test="matches(@key, ' ')">
                <xsl:apply-templates mode="#current"/>
                <xsl:variable name="key-parts" select="tokenize(@key, ' ')" as="xs:string*"/>
                <xsl:for-each select="$key-parts">
                    <a href="../../register/einrichtungen/detail.xql?id={.}">
                        <xsl:text> </xsl:text>
            ...
            <xsl:otherwise>
                <a href="../register/einrichtungen/detail.xql?id={$id}">
                    <xsl:apply-templates mode="#current"/>
                </a>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template mode="#all" match="tei:placeName">
        <xsl:choose>
            <xsl:when test="matches(@key, ' ')">
                ...
                <xsl:variable name="key-parts" select="tokenize(@key, ' ')" as="xs:string*"/>
                <xsl:for-each select="$key-parts">
                    <a href="../../register/orte/detail.xql?id={.}">
                        <xsl:text> </xsl:text>
                        <i class="far fa-map"/>
                        <xsl:if test="position() != last()">
                            <xsl:text> </xsl:text>
    ...
    <xsl:template mode="#all" match="tei:rs[@type='artwork']">
        <xsl:choose>
            <xsl:when test="matches(@key, ' ')">
                ...
                <xsl:variable name="key-parts" select="tokenize(@key, ' ')" as="xs:string*"/>
                <xsl:for-each select="$key-parts">
                    <a href="../../register/objekte/detail.xql?id={.}">
                        <xsl:text> </xsl:text>
    ...
    <xsl:template mode="#all" match="tei:bibl[@sameAs and not(ancestor::tei:seg and ancestor::tei:note)]">
        <xsl:choose>
            <xsl:when test="matches(@sameAs, ' ')">
                <xsl:apply-templates mode="#current"/>
                <xsl:variable name="sameAs-parts" select="tokenize(@sameAs, ' ')" as="xs:string*"/>
                <xsl:for-each select="$sameAs-parts">
                    <a href="../../register/werke/detail.xql?id={.}">
                        <xsl:text> </xsl:text>
...
<xsl:text>detail.xql?id=</xsl:text>
    ...
    <xsl:template mode="#all" match="tei:seg">
        <xsl:choose>
            <xsl:when test="$print='yes'">
                <span class="tooltip" id="{tei:note/@xml:id}">
