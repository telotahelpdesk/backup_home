...
let $tokenizedSearchTerms := <ul>
    {$param_q_title, $param_q_text, $param_q_head, $param_q_note
    }</ul>
...
let $query_text :=  '[ft:query(.//tei:text, $q_text)]'
...
let $query_date :=
    ...
    then (
        ...
        then ('[(.//tei:creation/tei:date/(@when|@from|@notBefore)/cs:startOfPeriod(.) >= $isoStartDate) and (.//tei:creation/tei:date/(@when|@to|@notAfter)/cs:endOfPeriod(.) <= $isoEndDate)]')
        (: "cross over" query, thx to Martin Fechner :)
        else ('[(.//tei:creation/tei:date/(@when|@from|@notBefore)/cs:startOfPeriod(.) <= $isoEndDate) and (.//tei:creation/tei:date/(@when|@to|@notAfter)/cs:endOfPeriod(.) >= $isoStartDate)]'))
    else (
        if ($isoStartDate)
        then ('[(.//tei:creation/tei:date/(@when|@from|@notBefore)/cs:startOfPeriod(.) >= $isoStartDate) or (.//tei:creation/tei:date/(@to|@notAfter)/cs:endOfPeriod(.) >= $isoStartDate)]')
        else (
            if ($isoEndDate)
            then ('[(.//tei:creation/tei:date/(@when|@from|@notBefore)/cs:startOfPeriod(.) <= $isoEndDate) or (.//tei:creation/tei:date/(@to|@notAfter)/cs:endOfPeriod(.) >= $isoStartDate)]')
            else ()
        )
    )

let $query_korrespondent := if ($q_korrespondent) then ('[.//tei:correspAction/tei:persName/@key=$q_korrespondent]') else ()
                            ...
                            then (concat('$collection//tei:div[@type="tag"]', $query_text, $query_note))
                                ...
                                then (concat('$collection//tei:back', $query_text, $query_note))
                                    ...
                                    then (concat('$collection//tei:TEI[@xml:id=$q_tageskalender]//(tei:div[@type="tag"]|tei:back)', $query_text, $query_note))
                                        ...
                                        else (concat('$collection//tei:TEI', $query_title, $query_abstract, $query_text, $query_head, $query_note, $query_vorlesungstyp, $query_korrespondent, $query_date))               
                                    )
                                )
                            )
                        )
                ...
                then (
                    ...
                    order by 
                        ...
                        then ($hit//tei:date/@when/data(.)) 
                            ...
                            then ($hit//tei:correspAction[@type='sent']/tei:date/(@when|@from|@notBefore)/data(.))
                            else ()
                        )
                    return 
                         ...
                         <div class="grid_6 box graybox">
                            <div class="boxInner">
                               ...
                               <p>Sie haben mit folgender Anfrage im Datenbestand gesucht:</p>
                                {if ($param_doctype='briefe')
                                then (edwebSearch:formBriefsuche($param_q_text, $param_q_title, $param_q_abstract, $param_q_note, $q_korrespondent, $isoStartDate, $isoEndDate))
                                else (
                                    if ($param_doctype='tageskalender')
...
let $output := edweb:transformHTML($xslt_input, 'suche', 'Suche')
