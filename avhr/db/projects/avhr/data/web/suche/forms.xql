...
declare function edwebSearch:formVorlesungssuche($param_q_head, $param_q_text, $param_q_note, $q_vorlesung, $q_vorlesungstyp){ 
    <form id="search" action="ergebnis.xql" accept-charset="utf-8">
        ...
        <label for="q_note">Sachanmerkungen</label>
...
declare function edwebSearch:formBriefsuche($param_q_text, $param_q_title, $param_q_abstract, $param_q_note, $q_korrespondent, $isoStartDate, $isoEndDate){   
    let $form :=
    <form id="search" action="ergebnis.xql" accept-charset="utf-8">
        <label for="q_text">Text</label>
            <input type="text" name="q_text" value="{$param_q_text}" />
        ...
        <label for="q_abstract">Regest</label>
            <input type="text" name="q_abstract" value="{$param_q_abstract}" />
        <label for="q_note">Sachanmerkungen</label>
            <input type="text" name="q_note" value="{$param_q_note}" />
        <label for="korrespondent">Korrespondent</label>
            <select name="korrespondent"><option value="">alle</option> 
            {
               for $key in distinct-values(collection($edweb:data)//tei:correspAction/tei:persName/@key)
               let $person := collection($edweb:dataRegister)//tei:person[@xml:id=$key]
               let $name := edweb:persName($person/tei:persName[@type='reg'], 'surname')
               order by $name
               return 
               if ($key=$q_korrespondent)
               then (<option selected="1" value="{$key}">{$name}</option>)
               else (<option value="{$key}">{$name}</option>)
            }
            </select>
