...
let $abfrage := 
    ...
    then (collection($edweb:dataRegister)//tei:person[ft:query(.//tei:persName, $lucenequery)][not(@role='mythological')])
    else (
        if ($p_register='orte')
        then (doc($edweb:dataRegisterPlaces)//tei:place[ft:query(./tei:placeName, $lucenequery)])
            ...
            then (doc($edweb:dataRegisterWorks)//tei:bibl[ft:query(., $lucenequery)])
                ...
                if ($p_register='myth')
                then (collection($edweb:dataRegister)//tei:person[ft:query(.//tei:persName, $lucenequery)][@role='mythological'])
                else (
                    if ($p_register='orgs')
                    then (collection($edweb:dataRegister)//tei:org[ft:query(.//tei:orgName, $lucenequery)])
                    else ()
                )
            )
        )
...
let $ergebnis := 
    for $x in $abfrage
    let $label :=
        if ($p_register='personen' or $p_register='myth')
        then (
