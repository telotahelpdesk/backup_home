...
declare variable $edweb:dataRegisterPeriodika := concat($edweb:dataRegister, '/Periodika.xml');
...
declare function edweb:persName($persName, $rendFirst) {
    ...
    then ($persName/tei:name/text())
        ...
        then (
            ...
            then (concat($persName/tei:surname/normalize-space(.), ', ', $persName/tei:forename/normalize-space(.)))
            else ($persName/tei:surname/normalize-space(.)))
        else (
            if ($persName/tei:forename/text())
            then (concat($persName/tei:forename/normalize-space(.), ' ', $persName/tei:surname/normalize-space(.)))
            else ($persName/tei:surname/normalize-space(.))
        )
    )
};
