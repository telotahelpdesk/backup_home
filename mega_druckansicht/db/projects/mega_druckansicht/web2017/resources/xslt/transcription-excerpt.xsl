<xsl:stylesheet xmlns:exist="http://exist.sourceforge.net/NS/exist" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:telota="http://www.telota.de" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://www.w3.org/1998/Math/MathML" exclude-result-prefixes="#all" version="2.0">
    <!-- ##### Strukturelemente ##### -->
    <xsl:param name="id"/>
    ...
    <xsl:template mode="#all" match="tei:pb">
        <span id="{@n}" class="folio" title="Foliierung des Archivs">
    ...
    <xsl:template mode="kritischerText lesetext faksimileText" match="tei:note[@resp]">
        ...
        <xsl:variable name="resp">
            <xsl:choose>
                <xsl:when test="@resp = '#author'"> des Autors </xsl:when>
                <xsl:when test="@resp = '#addressee'"> des Empfängers </xsl:when>
                <xsl:when test="@resp = '#humboldt'"> von Humboldt </xsl:when>
                <xsl:otherwise>
            ...
            <span class="label{$collapsed}">
                <xsl:choose>
                    <xsl:when test="@rend = 'sticked'">
                        <i class="fa fa-sticky-note" aria-hidden="true"> </i>Aufgeklebte Notiz
                            <xsl:value-of select="$resp"/>
                    </xsl:when>
                    <xsl:when test="@rend = 'sticked' and @type = 'auxCalc'">
                        <i class="fa fa-sticky-note" aria-hidden="true"> </i>Aufgeklebte Notiz mit
                        Nebenrechnung <xsl:value-of select="$resp"/>
                    </xsl:when>
                    <xsl:when test="@type = 'auxCalc'">
                        <i class="fa fa-chevron-circle-down" aria-hidden="true"> </i>Nebenrechnung
                            <xsl:value-of select="$resp"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <i class="fa fa-chevron-circle-down" aria-hidden="true"> </i>Anmerkung
                            <xsl:value-of select="$resp"/>
                    </xsl:otherwise>
                </xsl:choose>
                                ...
                                <xsl:for-each select="tei:cell">
                                    <th>
                                        <xsl:if test="@rendition = '#right'">
                                            <xsl:attribute name="class"> align-right
                                            </xsl:attribute>
                                        </xsl:if>
                                        <xsl:apply-templates mode="#current"/>
                                    </th>
    ...
    <xsl:template mode="kritischerText faksimileText comment" match="tei:unclear">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <span class="unclear"> [...]<span class="symbol">(?)</span>
                </span>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
        ...
        <xsl:choose>
            <xsl:when test="@quantity != '' and @unit != ''">
                <span class="gap">
                    <span class="place fussnote">
                        <span class="fussnotenpfeil"> </span>
                        <xsl:value-of select="telota:labels(@unit, @quantity)"/> unleserlich </span>
                    [...] </span>
            </xsl:when>
            <xsl:otherwise>
                <span class="gap"> [...] </span>
    ...
    <xsl:template mode="#all" match="tei:measure">
        ...
        <xsl:variable name="item" as="node()">
            <xsl:copy-of select="doc('xmldb:///db/apps/avhr/data/Register/Maße.xml')//tei:item[@xml:id = $unit]"/>
        </xsl:variable>
    ...
    <xsl:template mode="#all" match="tei:ref[@type = 'siglum']">
        <a id="{@xml:id}" class="ref-siglum" href="../register/siglen/index.xql">
            <xsl:apply-templates mode="#current"/>
        </a>
    </xsl:template>
            ...
            <xsl:otherwise>
                <span class="tooltip">
                    <span class="fussnote">
                        ...
                        <br/>
                        <a class="close" title="Schließen" aria-label="Schließen"> Schließen <i class="fa fa-times-circle" aria-hidden="true"/>
                        </a>
                    </span>
                    <xsl:apply-templates select="tei:orig" mode="#current"/>
                </span>
            ...
            <xsl:variable name="note">
                <xsl:choose>
                    <xsl:when test="preceding::tei:note[./preceding::tei:metamark[@spanTo = concat('#', $id)]]">
                        <xsl:for-each select="preceding::tei:note[./preceding::tei:metamark[@spanTo = concat('#', $id)]]"> 3 </xsl:for-each>
                    </xsl:when>
    ...
    <xsl:template match="tei:head">
        ...
        <xsl:variable name="level" select="count(tokenize($id, '\.'))"/>
        <xsl:element name="h{$level}">
            <xsl:attribute name="id" select="$id"/>
            <xsl:attribute name="class" select="'excerpt'"/>
            <xsl:apply-templates mode="#current"/>
        </xsl:element>
    </xsl:template>
