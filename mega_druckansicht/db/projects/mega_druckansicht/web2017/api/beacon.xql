...
let $normid :=
    ...
    then (
        for $normid in collection($edweb:dataRegister)//tei:person[tei:idno and matches(tei:idno, $authorityPrefix) and contains($correspondents, @xml:id)]/tei:idno[matches(., $authorityPrefix)]/text()
        return
        ...
        then (
            for $normid in collection($edweb:dataRegister)//tei:person[tei:idno and matches(tei:idno, $authorityPrefix) and contains($personsMentioned, @xml:id)]/tei:idno[matches(., $authorityPrefix)]/text()
            return
            concat(substring-after($normid, $authorityPrefix), ',')    
        )
        else (
            for $normid in collection($edweb:dataRegister)//tei:person/tei:idno[matches(., $authorityPrefix)]/text()
            return
            concat(substring-after($normid, $authorityPrefix), ',')
        )
    )
...
let $description := 
    if ($p_type='correspondents')
    then ('Alle Korrespondenzpartner von Karl Marx bzw. Friedrich Engels, von denen ein oder mehrere Brief(e) in der Datenbank enthalten sind')
    else (
        if ($p_type='personsMentioned')
...
#CONTACT: grabsch@bbaw.de
#INSTITUTION: Berlin-Brandenburgische Akademie der Wissenschaften
#NAME: MEGAdigital - Online-Angebot der historisch-kritischen Gesamtausgabe von Karl Marx und Friedrich Engels
