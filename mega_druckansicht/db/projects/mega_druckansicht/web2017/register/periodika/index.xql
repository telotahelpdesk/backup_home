...
let $werkliste := <ul>{
    for $werk in doc($edweb:dataRegisterPeriodika)//tei:bibl[starts-with(normalize-space(./tei:title), $edwebRegister:section)]
...
let $header :=  
    <transferContainer>
        ...
        <h1>Register der Periodika - {$edwebRegister:section}</h1>
        <p>{count($werkliste/li)} Zeitschriften und Periodika</p>
    </transferContainer>
