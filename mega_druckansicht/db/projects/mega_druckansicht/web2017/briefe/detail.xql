...
let $uri := 'http://megadigital.bbaw.de/briefe/detail.xql?id='||$letter/@xml:id

let $citationSuggest:=
    <div class="grid_11 box whitebox citation-suggest">
        <div class="boxInner">
            <h4>Zitiervorschlag</h4>
            <p>{$letterTitle}. In: Marx-Engels-Gesamtausgabe digital. Hg. von
            der Internationalen Marx-Engels-Stiftung. Berlin-Brandenburgische
            Akademie der Wissenschaften, Berlin. URL: <a href="{$uri}">{$uri}</a>. Abgerufen am {format-date(current-date(), '[D01].[M01].[Y0001]')}.</p>
        </div>
    </div>
...
let $content := 
    ...
    then (<transferContainer>
        {if ($letterText!='') then ($letterText, $citationSuggest)
        else
            if ($conjectured) 
            then ()
