<xsl:stylesheet xmlns:exist="http://exist.sourceforge.net/NS/exist" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:telota="http://www.telota.de" xmlns:tei="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="tei exist telota" version="2.0">
    <!-- ##### Strukturelemente ##### -->
    <xsl:param name="id"/>
   ...
   <xsl:template mode="kritischerText faksimileText" match="tei:p[@rendition='#mMM' and (ancestor::tei:p or ancestor::tei:note)]">
    ...
    <xsl:template mode="kritischerText faksimileText" match="tei:figure">
        <input type="submit" value="Image: {@facs}" style="background-color: white; border: 0px solid;"/>
    </xsl:template>
    ...
    <xsl:template mode="#all" match="tei:note[@type='editorial' and not(parent::tei:seg)]">
        <span class="editorial" style="font-style: italic">
            <xsl:apply-templates mode="#current"/>
        </span>
    </xsl:template>
        ...
        <table>
           <head>
            ...
            <span class="unterstrichen"> 
                Tab.:  <xsl:apply-templates mode="#current"/>
            </span>
           </xsl:when>
            </xsl:choose>
                </xsl:for-each>
                            ...
                            <xsl:for-each select="tei:cell">
                                <xsl:choose>
                                    ...
                                    <xsl:when test="@rows='4'">
                                     ...
                                     <xsl:when test="@rows='5'">
                                       <td rowspan="5">
                                           <xsl:apply-templates mode="#current"/>
                                      </td>
                                    </xsl:when>
                                  <xsl:when test="@rows='6'">
                                       <td rowspan="6">
                                           <xsl:apply-templates mode="#current"/>
                                      </td>
                                    </xsl:when>
                                     <xsl:when test="@rows='7'">
                                       <td rowspan="7">
                                           <xsl:apply-templates mode="#current"/>
                                      </td>
                                    </xsl:when>
    ...
    <xsl:template match="tei:hi[@rendition='#normal']" mode="#all">
        <span class="langschrift">
            <xsl:apply-templates mode="#current"/>
        </span>
    </xsl:template>

    <xsl:template match="tei:hi[@rendition='#sub']" mode="#all">
        <span class="tiefergestellt">
            <xsl:apply-templates mode="#current"/>
        </span>
    </xsl:template>
    
    <xsl:template match="tei:orig[@type='sz']" mode="#all">
        <span class="satzzeichen">
            <xsl:apply-templates mode="#current"/>
        </span>
    </xsl:template>

    <xsl:template match="tei:head[@type='editor']" mode="#all">
       <xsl:choose>
             <xsl:when test="tei:supplied[@reason='editorialOutline']">
                <span>
                    <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
    ...
    <xsl:template match="tei:head" mode="#all">
        ...
        <xsl:choose>
          ...
          <xsl:when test="parent::tei:div[@n='1']">
                <span class="h1">
                    (level h1:) <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
            <xsl:when test="parent::tei:div[@n='2']">
                <span class="h2">
                    (level h2:) <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
            <xsl:when test="parent::tei:div[@n='3']">
                <span class="h3">
                    (level h3:) <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
            <xsl:when test="parent::tei:div[@n='4']">
                <span class="h4">
                   (level h4:)  <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
            <xsl:when test="parent::tei:div[@n='5']">
                <span class="h5">
                   (level h5:)  <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
            <xsl:when test="parent::tei:div[@n='6']">
                <span class="h6">
                    (level h6:)<xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
            <xsl:when test="parent::tei:div[@n='7']">
                <span class="h7">
                   (level h7:) <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
             <xsl:when test="parent::tei:div[@n='8']">
                <span class="h8">
                   (level h8:) <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
              <xsl:when test="parent::tei:div[@n='9']">
                <span class="h9">
                   (level h9:) <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
             <xsl:when test="parent::tei:div[@n='10']">
                <span class="10">
                   (level h10:) <xsl:apply-templates mode="#current"/>
                </span>
            </xsl:when>
            </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="tei:head[@type='leftMargin']" mode="#all">
       <xsl:choose>
       <xsl:when test="@n='1'">
	 <span class="h1">
          (level h1:)   <xsl:apply-templates mode="#current"/>
   ...
   (level h10:)         <xsl:apply-templates mode="#current"/>
    ...
    <xsl:template mode="kritischerText faksimileText" match="tei:gap | tei:add | tei:del">
      ...
      <xsl:choose>
            ...
            <!--<xsl:number format="1" level="any"/>-->
                <xsl:number format="1" count="tei:add | tei:gap | tei:del" level="any"/>
                
            </xsl:variable>
            <xsl:variable name="nr">
            <!--<xsl:number format="a" level="any"/>-->
                <xsl:number format="a" count="tei:add | tei:gap | tei:del" level="any"/>
        ...
        <xsl:choose>
            <xsl:when test="@cert='low'">
                <span class="tooltip">
                    ...
                    <a href="https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/register/personen/detail.xql?id={$id}">
                        <xsl:apply-templates mode="#current"/>
                    </a>
                </span>
            </xsl:when>
    ...
    <xsl:template mode="#all" match="tei:orgName">
        <a href="https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/register/einrichtungen/detail.xql?id={./@key}">
            <xsl:apply-templates mode="#current"/>
        </a>
    </xsl:template>
    <xsl:template mode="#all" match="tei:placeName">
        <a href="https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/register/orte/detail.xql?id={./@key}">
            <xsl:apply-templates mode="#current"/>
        </a>
    </xsl:template>
    ...
    <xsl:template mode="#all" match="tei:bibl[@sameAs and not(ancestor::tei:seg and ancestor::tei:note)]">
        <a href="https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/register/werke/detail.xql?id={./@sameAs}">
            <xsl:apply-templates mode="#current"/>
        </a>
    </xsl:template>
    <xsl:template mode="#all" match="tei:bibl[@sameAs and ancestor::tei:seg and ancestor::tei:note]">
        <a class="bibl" href="https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/register/werke/detail.xql?id={./@sameAs}">
            <xsl:apply-templates mode="#current"/>
        </a>
    </xsl:template>
    <xsl:template mode="#all" match="tei:date">
        <a href="https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/zeit/zeitsuche_ergebnis.xql?datum={./@when}">
            <xsl:apply-templates mode="#current"/>
        </a>
    </xsl:template>
    ...
    <xsl:template mode="#all" match="tei:rs[@type='letter']">
        <xsl:choose>
            <xsl:when test="@type='erschlossenerBrief'">
                <span class="rs">
                    <xsl:apply-templates mode="#current"/>
                    <a class="erschlossenerBrief" href="https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/briefe/detail.xql?id={@key}" title="Zum erschlossenen Brief">
            ...
            <xsl:otherwise>
                <span class="rs">
                    <xsl:apply-templates mode="#current"/>
                    <a class="brief" href="https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/briefe/detail.xql?id={@key}" title="Zum Volltext des Briefes">
        ...
        <xsl:variable name="target">
            <xsl:choose>
                ...
                <xsl:when test="matches(@target, '/')">
                    <xsl:text>https://xmledit.bbaw.de/ediarum/rest/db/projects/mega/web2017/briefe/detail.xql?id=</xsl:text>
